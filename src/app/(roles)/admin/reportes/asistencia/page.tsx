...
export default async function ReporteAsistenciaPage({ searchParams }: PageProps) {
  const { periodoId } = await searchParams;
  const exportQs = new URLSearchParams({ tipo: "asistencia" });
      ...
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-text-secondary dark:text-gray-400">
            Tasas de asistencia por sección.
          </p>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-100 text-sm dark:divide-gray-800">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wide text-text-secondary dark:text-gray-400">
                  ...
                  <th className="px-4 py-3 text-right">Ausentes</th>
                  <th className="px-4 py-3 text-right">Tardanzas</th>
                  <th className="px-4 py-3 text-right">% Asistencia</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800/50">
                {filas.map((f) => {
                  const totalRegistros = f.totalAsistencias + f.totalAusencias + f.totalTardanzas;
                  const pct = totalRegistros > 0
                    ? ((f.totalAsistencias / totalRegistros) * 100).toFixed(1)
                    : null;
                  const pctNum = totalRegistros > 0 ? (f.totalAsistencias / totalRegistros) * 100 : null;
                  const colorPct =
                    pctNum === null
                      ? ""
                      : pctNum >= 75
                  ...
                  return (
                    <tr key={f.asignaturaId} className="hover:bg-primary/[0.02] dark:hover:bg-primary/5">
                      ...
                      <td className="px-4 py-3 text-right text-text-secondary dark:text-gray-400">{f.totalClases}</td>
                      <td className="px-4 py-3 text-right text-emerald-600 dark:text-emerald-400">{f.totalAsistencias}</td>
                      <td className="px-4 py-3 text-right text-red-600 dark:text-red-400">{f.totalAusencias}</td>
                      <td className="px-4 py-3 text-right text-amber-600 dark:text-amber-400">{f.totalTardanzas}</td>
                      <td className={`px-4 py-3 text-right font-semibold ${colorPct}`}>
                        {pct !== null ? `${pct}%` : "—"}
                      </td>
                    </tr>
                  );

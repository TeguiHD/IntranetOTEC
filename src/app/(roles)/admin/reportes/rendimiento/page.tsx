import { listarPeriodosParaReportes, reporteRendimientoAcademico } from "@/actions/reportes";

type PageProps = {
  searchParams: Promise<{ periodoId?: string }>;
};

export default async function ReporteRendimientoPage({ searchParams }: PageProps) {
  const { periodoId } = await searchParams;
  const exportQs = new URLSearchParams({ tipo: "rendimiento" });
  ...
  const [periodos, filas] = await Promise.all([
    ...
    reporteRendimientoAcademico(periodoId),
  ]);
    ...
    <section className="space-y-5">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          {periodoActual && (
            <p className="mt-1 text-sm text-text-secondary dark:text-gray-400">{periodoActual.nombre}</p>
          )}
        </div>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-100 text-sm dark:divide-gray-800">
              ...
              <tbody className="divide-y divide-gray-50 dark:divide-gray-800/50">
                {filas.map((f) => {
                  const promedio = f.promedioGeneral;
                  const colorPromedio =
                    promedio >= 4
                  ...
                  return (
                    <tr key={f.asignaturaId} className="hover:bg-primary/[0.02] dark:hover:bg-primary/5">
                      <td className="px-4 py-3 font-medium text-text-primary dark:text-gray-100">
                        {f.asignaturaNombre}
                        {f.cursoNombre && f.cursoNombre !== f.asignaturaNombre && (
                          <span className="block text-xs text-text-muted dark:text-gray-500">{f.cursoNombre}</span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3 text-text-secondary dark:text-gray-400">
                        {f.docenteNombre ? `${f.docenteNombre} ${f.docenteApellido ?? ""}`.trim() : "—"}
                      </td>
                      <td className="px-4 py-3 text-right text-text-secondary dark:text-gray-400">{f.totalMatriculas}</td>
                      <td className={`px-4 py-3 text-right font-semibold ${colorPromedio}`}>

...
import { TUTORIALES_ALUMNO, type Tutorial } from "@/lib/tutoriales";
...
export default function AlumnoTutorialesPage() {
  const principales = TUTORIALES_ALUMNO.filter((t) => t.categoria === "principal");
  const completo = TUTORIALES_ALUMNO.find((t) => t.categoria === "completo");
    ...
    <section className="space-y-6">
      ...
      <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {principales.map((t) => (
            <TutorialCard key={t.slug} tutorial={t} rol="alumno" />
          ))}
        </div>
      </div>
      ...
      {completo && completo.disponible && (
        <div>
          ...
          <Link href={`/alumno/tutoriales/${completo.slug}`} className="block">

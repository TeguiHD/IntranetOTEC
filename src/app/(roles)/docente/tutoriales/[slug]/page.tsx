...
import { TUTORIALES_DOCENTE, getTutorialBySlug, getTutorialesAdyacentes } from "@/lib/tutoriales";

export function generateStaticParams() {
  return TUTORIALES_DOCENTE.map((t) => ({ slug: t.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const tutorial = getTutorialBySlug(slug, "docente");
  return { title: tutorial?.titulo ?? "Tutorial" };
}

export default async function DocenteTutorialPage({
  params,
}: {
  ...
  const { anterior, siguiente } = getTutorialesAdyacentes(slug, "docente");

  return (
    <section className="space-y-5">
      <div className="flex items-center gap-3">
        <Link
          href="/docente/tutoriales"
      ...
      {(anterior || siguiente) && (
        <nav className="flex flex-col gap-3 sm:flex-row">
          {anterior ? (
            <Link
              href={`/docente/tutoriales/${anterior.slug}`}
          ...
          {siguiente ? (
            <Link
              href={`/docente/tutoriales/${siguiente.slug}`}
